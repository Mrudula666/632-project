...
## Planet equity, pl_orbsMax and P
...
# Adjust the plotting window size
quartz(width = 15, height = 10)

# Define the features you want to compare against density
features <- c('st_mass', 'st_lum', 'st_teff', 'pl_bmasse', 'pl_rade', 'sy_kmag', 'grav_acc', 'pl_orbsmax', 'pl_orbper')

# Determine the number of plots per row and number of rows
num_plots_per_line <- 3
num_features <- length(features)
num_rows <- ceiling(num_features / num_plots_per_line)

par(mfrow = c(num_rows, num_plots_per_line), mar = c(4, 4, 2, 1)) 


# Save the plots to a PNG file
png("density_vs_features.png", width = 2000, height = 1200, res = 200)

# Create the plots
par(mfrow = c(num_rows, num_plots_per_line), mar = c(4, 4, 2, 1))
for (feature in features) {
  plot(
    exoplanet_data$pl_dens, exoplanet_data[[feature]],
    xlab = 'Planet Density (g/cm³)',
    ylab = feature,
    main = paste('Planet Density vs.', feature),
    pch = 19,
...
vars_to_remove_backward <- c("sy_snum", "pl_orbper", "pl_radj", "pl_bmassj", "pl_dens", "pl_bmassprov", "sy_gaiamagerr2", "decstr","rastr","st_logg", "pl_name","hostname", "sy_kmag", "disc_telescope", "disc_facility", "sy_gaiamag", "sy_pnum","ttv_flag","sy_snum","ra","dec","st_lum","st_mass","pl_bmasse")


# Remove one variable from each pair of highly correlated variables
exoplanet_data_reduced <- exoplanet_data[, !colnames(exoplanet_data) %in% vars_to_remove_corelation_vif]
...
```{r}
# Plot the ROC curve
plot(rf_roc_obj, main = "ROC Curve for Random Forest", col = "#FF0000", lwd = 2)

# Optionally, add text to show the AUC on the plot
rf_auc_value <- auc(rf_roc_obj)
text(0.6, 0.2, paste("AUC =", round(rf_auc_value, 3)), cex = 1.2)
```
